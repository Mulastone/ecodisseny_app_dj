document.addEventListener("DOMContentLoaded", function () {
  ...
  if (addBtn) {
    addBtn.addEventListener("click", function () {
      ...
      const firstLinea = container.querySelector(".pressupost-linea");
      if (!firstLinea) return;

      const newLinea = firstLinea.cloneNode(true);
      newLinea.classList.add("pressupost-linea");

      newLinea.querySelectorAll("input, select, textarea, label").forEach((el) => {
        if (el.name) el.name = el.name.replace(/-\d+-/, `-${currentIndex}-`);
        if (el.id) el.id = el.id.replace(/-\d+-/, `-${currentIndex}-`);
        if (el.tagName !== "LABEL") {
      ...
      const oldDeleteBtn = newLinea.querySelector(".delete-linea");
      if (oldDeleteBtn) oldDeleteBtn.remove();

      const deleteBtn = document.createElement("button");
      deleteBtn.type = "button";
      deleteBtn.className = "btn btn-outline-danger btn-sm eliminar-linea";
      deleteBtn.innerHTML = "🗑️";
      deleteBtn.addEventListener("click", () => {
        newLinea.remove();
        calcularTotalPressupost();
      });
      const lastCell = newLinea.querySelector("td:last-child");
      if (lastCell) lastCell.appendChild(deleteBtn);
  ...
  function setupLinea(linea, index) {
    ...
    // Recalcular al cambiar el porcentaje de beneficio
    ...
    function calcularSubtotal() {
      ...
      // Usa el porcentaje introducido por el usuario
      ...
      // El campo beneficiField muestra el porcentaje, no el valor absoluto
      // Si quieres mostrar el valor absoluto, crea un campo aparte
